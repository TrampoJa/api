...
class OfertasSerializer(serializers.ModelSerializer):
    ...
    estabelecimento = serializers.ReadOnlyField(
        source='owner.estabelecimento.nome')
    estabelecimento_id = serializers.ReadOnlyField(
        source='owner.estabelecimento.id')
    logo = serializers.ImageField(
        source='owner.estabelecimento.logo', allow_null=True, read_only=True)
    telefone = serializers.ReadOnlyField(
        source='owner.estabelecimento.telefone')
    ...
    def get_avaliacao(self, oferta):
        avaliacao = Avaliacoes.objects.filter(
            owner_id=oferta.owner.id).aggregate(Avg('nota'))
